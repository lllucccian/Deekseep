...
final class RemoteFeatureFlags {
    ...
    /*
     * These are DeepSeek's own Boolean feature settings present in every supported APK. Some
     * consumers are inlined or observe the internal settings state rather than repeating the MMKV
     * key literal, so a low string-reference count must not be used to hide a working native flag.
     */
    ...
    private static Feature feature(String suffix, String zh, String en,
        ...
        return new Feature("kv_remote_settings_" + suffix, zh, en, detailZh, detailEn, false);
    }

    private static Feature invertedFeature(String suffix, String zh, String en,
            String detailZh, String detailEn) {
        return new Feature("kv_remote_settings_" + suffix, zh, en, detailZh, detailEn, true);
    }
    ...
    static boolean resetAll(ClassLoader loader) {
        ...
        synchronized (LOCK) {
            ...
            try {
                SharedPreferences.Editor editor = preferences.edit();
                for (Feature feature : FEATURES) editor.remove(localKey(feature.key));
                if (!editor.commit()) return false;
            } catch (Throwable error) {
