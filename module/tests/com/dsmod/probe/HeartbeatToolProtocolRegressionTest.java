...
public final class HeartbeatToolProtocolRegressionTest {
    ...
    static final class V234Message {
        String h = "USER";
        List<Object> n = new ArrayList<>();
        final List<FoldFragment> fragments;
        V234Message(String content) {
            fragments = new ArrayList<>(Arrays.asList(
                    new FoldFragment("REQUEST", content)));
        }
        public List<FoldFragment> r() { return fragments; }
    }
    ...
    public static void main(String[] args) {
        ...
        // Exercise the real SSE shape: the marker and JSON arrive a few characters at a time.
        // No prefix byte, escaped marker byte, or JSON byte may ever enter the visible frame.
        String streamingPrefix = "\u6b63\u5728\u5904\u7406\u3002\n";
        String streamedBlock = singleCallBlock(
                "{\"id\":\"char_stream_001\",\"tool\":\"get_current_time\","
                        + "\"scope\":\"conversation-1234\"}");
        for (int length = 1; length < streamedBlock.length(); length++) {
            String frame = HeartbeatToolProtocol.parseForConversation(
                    streamingPrefix + streamedBlock.substring(0, length)).visibleText;
            require(streamingPrefix.equals(frame),
                    "character-streamed private control data became visible at byte " + length);
        }
        String escapedOpening = "\\[\\[DEEKSEEP\\_LOCAL\\_TOOLS\\_V1\\]\\]";
        for (int length = 1; length < escapedOpening.length(); length++) {
            String frame = HeartbeatToolProtocol.parseForConversation(
                    streamingPrefix + escapedOpening.substring(0, length)).visibleText;
            require(streamingPrefix.equals(frame),
                    "character-streamed escaped marker became visible at byte " + length);
        }
        HeartbeatToolProtocol.Result completedCharacterStream =
                HeartbeatToolProtocol.parseForConversation(
                        streamingPrefix + streamedBlock);
        require(completedCharacterStream.calls.size() == 1
                        && !completedCharacterStream.visibleText.contains("DEEKSEEP")
                        && !completedCharacterStream.visibleText.contains("char_stream_001"),
        ...
        String networkBlock = singleCallBlock(
                "{\"id\":\"net_001\",\"tool\":\"network_request\","
                        + "\"scope\":\"conversation-1234\","
                        + "\"url\":\"https://example.com/api?q=one\","
                        + "\"method\":\"POST\",\"headers\":{"
                        + "\"Accept\":\"application/json\","
                        + "\"X-Test\":\"a'b\"},\"body\":\"{\\\"ok\\\":true}\","
                        + "\"timeout_ms\":12000}");
        HeartbeatToolProtocol.Result networkParsed =
                HeartbeatToolProtocol.parseForConversation(networkBlock);
        require(networkParsed.calls.size() == 1
                        && HeartbeatToolProtocol.TOOL_NETWORK_REQUEST.equals(
                        networkParsed.calls.get(0).tool)
                        && "POST".equals(networkParsed.calls.get(0).mode)
                        && networkParsed.calls.get(0).instruction.contains(
                        "X-Test: a'b")
                        && networkParsed.calls.get(0).content.contains("\"ok\":true"),
                "structured network request was not parsed");
        String curl = AgentDeviceBridge.networkCurlCommand(
                networkParsed.calls.get(0));
        require(curl.contains("curl --silent --show-error --location")
                        && curl.contains("--request 'POST'")
                        && curl.contains("--header 'X-Test: a'\\''b'")
                        && curl.endsWith("'https://example.com/api?q=one'"),
                "network request did not become a safely quoted curl command");
        require(HeartbeatToolProtocol.parse(singleCallBlock(
                        "{\"id\":\"net_bad\",\"tool\":\"network_request\","
                                + "\"scope\":\"conversation-1234\","
                                + "\"url\":\"file:///data/local/tmp/x\"}"))
                        .calls.isEmpty(),
                "network request accepted a non-HTTP URL");
        require(HeartbeatToolProtocol.parse(singleCallBlock(
                        "{\"id\":\"net_hdr_bad\",\"tool\":\"network_request\","
                                + "\"scope\":\"conversation-1234\","
                                + "\"url\":\"https://example.com\","
                                + "\"headers\":{\"X-Test\":\"ok\\r\\nInjected: yes\"}}"))
                        .calls.isEmpty(),
                "network request accepted a CRLF-injected header");
        AgentToolConfig.Snapshot v6Migrated = AgentToolConfig.decode(
                "{\"version\":6,\"enabled\":true,\"enabled_tools\":[\"ask_user\"]}");
        require(v6Migrated.enabledTools.contains(
                        HeartbeatToolProtocol.TOOL_NETWORK_REQUEST),
                "v6 settings did not enable the new network tool during migration");
        ...
        try {
            ...
            Field v234Field = HostCompat.class.getDeclaredField("v234");
            Field v230Field = HostCompat.class.getDeclaredField("v230");
            v234Field.setAccessible(true);
            v230Field.setAccessible(true);
            boolean previousV234 = v234Field.getBoolean(null);
            boolean previousV230 = v230Field.getBoolean(null);
            try {
                v234Field.setBoolean(null, true);
                v230Field.setBoolean(null, true);
                Method hiddenTransport = Main.class.getDeclaredMethod(
                        "isHiddenAgentTransportUserMessage", Object.class);
                hiddenTransport.setAccessible(true);
                V234Message v234Result = new V234Message(
                        "<system>\nprivate\n</system>\n\n" + toolResult);
                require(Boolean.TRUE.equals(hiddenTransport.invoke(null, v234Result)),
                        "2.3.4 r() message fragments did not hide the tool result");
            } finally {
                v234Field.setBoolean(null, previousV234);
                v230Field.setBoolean(null, previousV230);
            }
