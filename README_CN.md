...
本版包含基础 Agent 工具（具体工具请在应用内查看/下载）、自动继续生成、可见的“回复完成通知”开关、本地禁言、自定义主页欢迎语、自定义 DeepSeek 头像（需先在“灰度功能管理器”开启“显示助手头像”）、鲸鱼旋转和深海文字波纹、屏幕 Hook 日志、崩溃记录/测试、自定义请求、禁用热更新、自动清理缓存、进程管理器，以及本地 API 前台心跳保活和长上下文自动转文件上传。后台完成回复后会发系统通知；通知开关位于“自动继续生成”旁边。仍建议在电池优化中不要限制 DeepSeek。
...
首页只推荐维护中的通用版；已废弃的 API 专用包和诊断包不再是构建目标。

<details>
<summary>维护中的安装包</summary>

| APK 或源码类型 | 适用场景 | Xposed 接口 | 支持状态与日志 |
|---|---|---|---|
| `Deekseep.apk` | DeepSeek 2.2.0/2.3.0 和 2.3.4 国内版/Google Play | 传统 Xposed 兼容通用入口 | 当前维护的融合版；可选诊断默认关闭。 |
