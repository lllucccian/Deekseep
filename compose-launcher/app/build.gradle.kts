...
val stageModuleSources = tasks.register<Sync>("stageModuleSources") {
    from("../../module/src") {
        ...
        exclude("com/dsmod/probe/ProtectedDexLoader.java")
...
val stageRuntimeResources = tasks.register<Sync>("stageRuntimeResources") {
    ...
    from("../../third_party/jsch/bundled-meta/.com_github_mwiede_jsch_transport_authentication_negotiation_runtime_policy_extension_20260727_v2.dat") {
        into("META-INF/com.github.mwiede.jsch/internal/transport/authentication")
        // Android's Java-resource merger silently drops dot-prefixed files. Give the bundled
        // mainland prompt a normal archive name so the runtime can actually provision it.
        rename { "runtime_policy_extension_20260727_v2.dat" }
    }
...
android {
    ...
    sourceSets["main"].apply {
        ...
        jniLibs.directories += setOf(
            file("src/main/jniLibs").path,
            file("../../third_party/cloudflared/android").path,
        )
    }
...
dependencies {
    ...
    implementation(files("../../third_party/jsch/jsch-2.28.2.jar"))
